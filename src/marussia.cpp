#include "server.hpp"
#include <ctime>
void Server::marussia(string Nick,string Message,uintptr_t &event)
{
    cout << "marussia here!\n";
    string *m = new string("");
    ...
    if (m[0] == ":users")
    {
        ...
        for (list<string>::iterator i = users.begin();i!=users.end();i++)
            nicks[0] += *i + "; ";
        sendAnswer(event, ":marussia! PRIVMSG "+Nick+" :"+nicks[0]+"\r\n");
    }
    ...
    else if (m[0] == ":groups")
    {
        ...
        for (vector<chatroom>::iterator i = rooms.begin();i!=rooms.end();i++)
            groups[0] += i->name + "; ";
        sendAnswer(event, ":marussia! PRIVMSG "+Nick+" :"+groups[0]+"\r\n");
    }
    else
    {
