...
void Server::cmdQUIT(uintptr_t &event)
{
    list<string>::iterator it2 = users.begin();
    for (list<uintptr_t>::iterator it = fds.begin();it!=fds.end() || it2!=users.end();it++, it2++)
    {
        if (*it == event)
        {
            if (users.size() == 1)
...
void Server::cmdPRIVMSG(string &str, struct kevent &e)
{
    ...
    list<uintptr_t>::iterator it2 = fds.begin();
    for (int i = space + 1;str[i] != ' ';i++)
        nick[0] += str[i];
    for (unsigned long i = m;i<str.size();i++)
        message[0]+=str[i];
    int j = 0;uintptr_t *event = new uintptr_t;
    for (list<string>::iterator i = users.begin();i != users.end();i++)
    {
        ...
        if (nick[0] != "")
        {
            ...
            else
            {
                ...
                for (size_t i = 0;i<n.size();i++)
                {
                    ...
                    for (list<uintptr_t>::iterator it = r.Fds.begin();it != r.Fds.end();it++)
                    {
...
void Server::cmdPART(string &str, struct kevent &event)
{
    ...
    for (vector<string>::iterator i = names.begin();i!=names.end();i++)
    {
        for (vector<chatroom>::iterator it = rooms.begin();it!=rooms.end();it++)
        {
            if (*i == it->name)
            {
                list<uintptr_t>::iterator FDS = it->Fds.begin();
                cout << "=\n";
